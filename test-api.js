...
async function testAPI() {
  ...
  try {
    ...
    if (loginResponse.data.success) {
      ...
      console.log(`✅ Agents endpoint working. Found ${agentsResponse.data.agents.length} agents`);
      ...
      const createAgentResponse = await axios.post(`${API_BASE_URL}/api/agents`, newAgent);
      if (createAgentResponse.data.success) {
        ...
        console.log(`✅ Lists endpoint working. Found ${listsResponse.data.lists.length} lists`);
        ...
        console.log(`   • Total Agents: ${agentsResponse.data.agents.length + 1}`);
